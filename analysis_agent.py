...
def check_api_status() -> tuple[bool, str]:
    ...
    try:
        genai.configure(api_key=api_key)
        model = genai.GenerativeModel('gemini-2.5-flash') # 接続確認には軽量モデルを使用
...
def translate_news_batch(news_items: list[dict], model_id: str = None) -> list[dict]:
    """
    ニュース記事のリスト（タイトル・要約）を一括で日本語翻訳する
    ...
    prompt = f"""
あなたはプロの翻訳家です。以下の金融ニュースのタイトルと要約を自然な日本語に翻訳してください。
金融用語を適切に使用し、投資家にとって読みやすい、簡潔な文章にしてください。

以下の形式のJSONリストのみを出力してください（Markdownコードブロックは不要、JSONデータのみ）。
[
  {{ "index": 0, "title_jp": "翻訳されたタイトル", "summary_jp": "翻訳された要約" }},
  ...
]
...
"""
    ...
    try:
        ...
        # 結果をマージ
        result_map = {item['index']: item for item in translated_data}
        
        translated_items = []
        for i, item in enumerate(news_items):
            new_item = item.copy()
            if i in result_map:
                if 'title_jp' in result_map[i] and result_map[i]['title_jp']:
                    new_item['title'] = result_map[i]['title_jp']
                if 'summary_jp' in result_map[i] and result_map[i]['summary_jp']:
                    new_item['summary'] = result_map[i]['summary_jp']
